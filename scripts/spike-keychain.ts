...
function powershellSource(operation: "store" | "retrieve" | "remove", accountName: string, resourceName: string, envelope: string): string {
  ...
  return `$envelope = @'\n${envelope}\n'@\nAdd-Type -AssemblyName System.Runtime.WindowsRuntime; $null = [Windows.Security.Credentials.PasswordVault,Windows.Security.Credentials,ContentType=WindowsRuntime]; $vault = New-Object Windows.Security.Credentials.PasswordVault; $resource = ${resourceLiteral}; $account = ${accountLiteral}; try { if ("${operation}" -eq "store") { try { $old = $vault.Retrieve($resource, $account); $vault.Remove($old) } catch {}; $secret = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($envelope.Substring(5))); $credential = New-Object Windows.Security.Credentials.PasswordCredential -ArgumentList $resource,$account,$secret; $vault.Add($credential) } elseif ("${operation}" -eq "retrieve") { [Console]::Error.WriteLine("PasswordVault retrieve branch"); $credential = $vault.Retrieve($resource, $account); $credential.RetrievePassword(); if ($null -eq $credential.Password) { throw "PasswordVault returned a null password" }; [Console]::Error.WriteLine("PasswordVault retrieval length=" + $credential.Password.Length); $encoded = "blv1:" + [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($credential.Password)); [Console]::WriteLine($encoded) } elseif ("${operation}" -eq "remove") { $credential = $vault.Retrieve($resource, $account); $vault.Remove($credential); [Console]::Error.WriteLine("PasswordVault remove completed") } else { throw "unknown PasswordVault operation" }; exit 0 } catch { [Console]::Error.WriteLine($_.Exception.GetType().FullName); [Console]::Error.WriteLine($_.Exception.Message); exit 1 }`;
}
