...
describe("atomic config store", () => {
  ...
  it("never writes a raw API secret to the connection file", async () => {
    ...
    roots.push(root);
    const store = new ConnectionStore(join(root, "state-root"), new FakeKeychain());
    await store.save("https://example.test/api/v1", "bl_store_secret");
    expect(await readFile(store.paths.connection, "utf8")).not.toContain("bl_store_secret");
    expect(await readFile(store.paths.connection, "utf8")).toContain("keychain");
  });
});
