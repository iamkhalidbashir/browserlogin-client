...
import {
  startDistributionMock,
  verifyTestOnlyChecksums,
} from "../mocks/distribution-server.js";
...
describe("local mock servers", () => {
  it("covers BrowserLogin REST success and named 401/409/412/422 paths", async () => {
    const server = await startBrowserLoginMock();
    closers.push(server.close);
    const ok = await fetch(`${server.url}/api/v1/profiles`, {
      headers: { Authorization: "Bearer bl_test_key_secret" },
    });
    expect(ok.status).toBe(200);
    expect(
      (
        (await ok.json()) as Array<{
          id: string;
          name: string;
          platform: string;
        }>
    ...
    const startResponse = await fetch(
      `${server.url}/api/v1/profiles/profile-1/sessions`,
      {
        method: "POST",
        headers: {
          Authorization: "Bearer bl_test_key_secret",
    ...
    const started = (await startResponse.json()) as {
      session: { id: string; profile_id: string; state: string };
      archive: {
        profile_id: string;
        generation: number;
        size: number;
    ...
    expect(started.archive).toMatchObject({
      profile_id: "profile-1",
      generation: 4,
      size: 4,
      sha256:
    ...
    const statusResponse = await fetch(
      `${server.url}/api/v1/sessions/session-1/status`,
      {
        headers: {
          Authorization: "Bearer bl_test_key_secret",
          Accept: "application/json",
    ...
    const stopResponse = await fetch(
      `${server.url}/api/v1/sessions/session-1/stop`,
      {
        method: "POST",
        headers: {
          Authorization: "Bearer bl_test_key_secret",
    ...
    const download = await fetch(
      `${server.url}/api/v1/profiles/profile-1/archive/download?generation=4`,
      {
        headers: {
          Authorization: "Bearer bl_test_key_secret",
          "If-Match":
    ...
    const uploadUrl = await fetch(
      `${server.url}/api/v1/profiles/profile-1/archive-upload-url`,
      {
        method: "POST",
        headers: {
          Authorization: "Bearer bl_test_key_secret",
    ...
    expect(
      ((await uploadUrl.json()) as { upload_url: string }).upload_url,
    ).toBe("https://convex-storage.test/api/storage/upload/session-1");
  });
    ...
    expect(
      (
        await post({
          ...
          id: 3,
          method: "tools/call",
          params: { name: "profiles_list", arguments: {} },
        })
      ).status,
    ).toBe(200);
  ...
  it("serves signed per-platform archives and fails the post-signing tamper check", async () => {
    const server = await startDistributionMock();
    closers.push(server.close);
    const sumsResponse = await fetch(`${server.url}/SHA256SUMS`);
    const sums = (await sumsResponse.json()) as {
      checksums: string;
      signature: string;
      publicKey: string;
      label: string;
    ...
    const publicKey = createPublicKey({
      key: Buffer.from(sums.publicKey, "base64"),
      format: "der",
      type: "spki",
    });
    expect(
      verify(
        null,
        Buffer.from(sums.checksums),
        publicKey,
        Buffer.from(sums.signature, "base64"),
      ),
    ).toBe(true);
    const archive = Buffer.from(
      await (
        await fetch(`${server.url}/archives/darwin-arm64.zip`)
      ).arrayBuffer(),
    );
    ...
    const pro = await fetch(`${server.url}/pro/browser-archive`, {
      headers: { Authorization: "Bearer bl_test_key_secret" },
    });
    expect(pro.status).toBe(200);
    const missingBearer = await fetch(`${server.url}/api/download/v1.0.0`, {
    ...
    const download = await fetch(`${server.url}/api/download/v1.0.0`, {
      headers: {
        Authorization: "Bearer bl_test_key_secret",
    ...
    expect(
      (
        (await (
          await fetch(`${server.url}/api/download/v1.0.0`, {
            headers: { Authorization: "Bearer bl_test_key_secret" },
          })
